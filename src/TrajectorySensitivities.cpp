...
std::vector<dwddronep> DroneTrajectory::trajSensDroneParam(SimResults const & simResults, int iterations)
{
    std::vector<dwddronep> ts;
    ts.reserve(iterations);

    Eigen::Matrix<double, NUM_PLANT_STATES, NUM_DRONE_PARAMS> initial_dxdp;
    initial_dxdp.setZero();
    Eigen::Matrix<double, NUM_Z_STATES, NUM_DRONE_PARAMS> initial_dzdp;
    initial_dzdp.setZero();
    Eigen::Matrix<double, NUM_Y_STATES, NUM_DRONE_PARAMS> initial_dydp;
    initial_dydp.setZero();
    ts.emplace_back(initial_dxdp, initial_dzdp, initial_dydp);
    ...
    Eigen::Matrix<double, NUM_PLANT_STATES, NUM_PLANT_STATES> dfdx_curr;
    Eigen::Matrix<double, NUM_PLANT_STATES, NUM_PLANT_STATES> A;
    Eigen::Matrix<double, NUM_PLANT_STATES, NUM_DRONE_PARAMS> B;
    Eigen::PartialPivLU<Eigen::Matrix<double, NUM_PLANT_STATES, NUM_PLANT_STATES>> solver;

    Eigen::Matrix<double, NUM_PLANT_STATES, NUM_DRONE_PARAMS> dxdp_plus;
    Eigen::Matrix<double, NUM_Z_STATES, NUM_DRONE_PARAMS> dzdp_plus;
    Eigen::Matrix<double, NUM_Y_STATES, NUM_DRONE_PARAMS> dydp_plus;
    ...
    for(int i = 1; i < iterations; i++)
    {
        double timestep = simResults.time[i] - simResults.time[i-1];
        // dwdwo
        dwddronep curr = ts[i-1];
        dxdp = curr.dxdp;
        dzdp = curr.dzdp;

        SystemState currState = simResults.stateProgression[i];
        SystemState prevState = simResults.stateProgression[i-1];

        // dxdwo_plus
        dfdx_plus = dfdx({currState.plant, prevState.alge});
        dfdx_curr = dfdx(prevState);
        Eigen::SparseMatrix<double> dfdz_plus = dfdz({currState.plant, prevState.alge}); 
        Eigen::SparseMatrix<double> dfdz_curr = dfdz(prevState);

        Eigen::SparseMatrix<double> dfddronep_plus = dfddronep({currState.plant, prevState.alge}); 
        Eigen::SparseMatrix<double> dfddronep_curr = dfddronep(prevState);

        A = I - (timestep / 2.0) * dfdx_plus;
        B = dxdp + (timestep / 2.0)*(dfdx_curr*dxdp + (dfdz_plus+dfdz_curr)*dzdp + dfddronep_plus + dfddronep_curr );
        solver.compute(A);
        dxdp_plus.noalias() = solver.solve(B);
        ...
        dzdp_plus = dhdx_plus * dxdp_plus + dhdx_curr * dxdp + dhdz_curr * dzdp;

        for(int zBeforey = 1; zBeforey <= desThrust; zBeforey ++){
            Eigen::Matrix<double, 1, NUM_DRONE_PARAMS> tmp;
            tmp = dhdz_plus.block(zBeforey, 0, 1, zBeforey) * dzdp_plus.topRows(zBeforey);
            dzdp_plus.row(zBeforey) += tmp;
        }  
        
        // dydwo
        dydp_plus = dgdz_plus * dzdp_plus + dgdx_plus * dxdp_plus;
        ...
        Eigen::SparseMatrix<double> dhddronep_plus = dhddronep(simResults.stateProgression[i]);
        dzdp_plus += dhddronep_plus;
